...
# --- 設定區 ---
TICKET_CATEGORY_ID = 1487097289041379488  # 客服單分類 ID
ALLOWED_GUILD_ID = 123456789012345678    # 【重要】請替換成你的伺服器 ID
ALLOWED_CHANNEL_ID = 123456789012345678  # 【重要】請替換成你准許發送指令的頻道 ID
...
class ApplyTypeView(discord.ui.View):
    ...
    @discord.ui.select(
        ...
        custom_id="persistent:apply_select"
    )
...
class TicketLauncher(discord.ui.View):
    ...
    async def create_ticket(self, interaction: discord.Interaction, category: str):
        ...
        try:
            ...
            await interaction.response.send_message(f"❌ 建立客服單時出錯: {e}", ephemeral=True)
...
class TicketCog(commands.Cog):
    ...
    @app_commands.checks.has_permissions(administrator=True) # 第一重：必須是管理員
    async def setup_ticket(self, interaction: discord.Interaction):
        # 第二重：檢查伺服器 ID
        if interaction.guild_id != ALLOWED_GUILD_ID:
            return await interaction.response.send_message("❌ 此指令不支援在此伺服器使用。", ephemeral=True)

        # 第三重：檢查頻道 ID
        if interaction.channel_id != ALLOWED_CHANNEL_ID:
            return await interaction.response.send_message(
                f"❌ 安全檢查失敗：請前往 <# {ALLOWED_CHANNEL_ID}> 執行此指令。", 
                ephemeral=True
            )
        ...
        embed = discord.Embed(
            title="🎫 OHW 客服系統",
            description=(
                "如果您遇到遊戲問題、儲值問題或檢舉玩家，請點擊下方按鈕開啟客服單\n\n"
                "**YouTube Rank 申請資格:**\n"
                "1. 需要有 1000 訂閱\n"
                "2. 至少 1 部影片 (不包含 Shorts) 擁有 1000 觀看\n"
                "3. 需要拍一部宣傳片，且觀看需擁有 500 觀看 (Shorts 不算)\n\n"
                "如果您有任何問題，請隨時聯繫我們的客服團隊！"
            ),
            color=discord.Color.gold()
        )
        await interaction.response.send_message("✅ 驗證成功，面板已送出。", ephemeral=True)
